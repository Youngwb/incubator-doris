...
template<typename DstRowType, typename SrcRowType>
void agg_update_row_with_replace_version_column(DstRowType* dst, const SrcRowType& src, uint32_t replace_version_cid, MemPool* mem_pool) {
    for (uint32_t cid = dst->schema()->num_key_columns(); cid < dst->schema()->num_columns(); ++cid) {
        auto dst_cell = dst->cell(cid);
        auto src_cell = src.cell(cid);
...
template<typename DstRowType, typename SrcRowType>
void agg_update_row_with_replace_version_column(const std::vector<uint32_t>& cids, DstRowType* dst, const SrcRowType& src, uint32_t replace_version_cid) {
    for (auto cid : cids) {
        auto dst_cell = dst->cell(cid);
        auto src_cell = src.cell(cid);
        if (dst->column_schema(cid)->aggregation() == OLAP_FIELD_AGGREGATION_REPLACE) {
